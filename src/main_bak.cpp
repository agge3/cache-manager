...
TEST(SinglyLinkedListTest, BasicOperationsInt) {
	...
	for (auto i = list.size(); i > 0; --i) {
		...
		EXPECT_EQ(*front, *v)
			<< "Front: " << *front << ", Popped value: " << *v << "\n";
		EXPECT_TRUE(list.front() != *front)
			<< "Current front: " << *list.front() << ", Old front: " << *front
			<< ", List size: " << list.size() << "\n";
		EXPECT_EQ(list.size(), i - 1);
	}
	EXPECT_TRUE(list.isEmpty());
...
class HashNodeTest : public testing::Test {
  protected:
	HashNodeTest()
		: _node1(1, 1), _node2(1), _node3("key", 3), _node4("key"),
		  _node5(2, 2), _node6(3) {
		// do nothing
	}
...
TEST(HashMapMembers, add) {
	...
	EXPECT_TRUE(
		strIntMap->contains("Very long string with spaces and mixed case."));
	...
	EXPECT_FALSE(strIntMap->contains(
		"Another very long string with spaces and mixed case."));
	...
	EXPECT_TRUE(
		*(strIntMap->getItem("Very long string with spaces and mixed case.")) ==
		0x7fffffff);
...
template <typename K> struct NoHash {
...
class CacheManagerTest : public testing::Test {
  protected:
	CacheManagerTest()
		: _capacity(10), _intCache(_capacity), _emptyIntCache(_capacity),
		  _strCache(_capacity), _emptyStrCache(_capacity),
		  _strIntCache(_capacity), _emptyStrIntCache(_capacity) {
		_intCache.add(1, 0);
		_intCache.add(2, 1);
		_intCache.add(3, 3);
...
TEST_F(CacheManagerTest, getItem) {
	...
	EXPECT_EQ(
		*_strIntCache.getItem("Very long string with spaces and mixed case."),
		0x7fffffff);
	EXPECT_FALSE(_emptyStrIntCache.getItem("key").has_value());
	EXPECT_FALSE(_emptyStrIntCache.getItem("k").has_value());
	EXPECT_FALSE(_emptyStrIntCache
					 .getItem("Very long string with spaces and mixed case.")
					 .has_value());
}
...
TEST_F(CacheManagerTest, contains) {
	...
	EXPECT_TRUE(
		_strIntCache.contains("Very long string with spaces and mixed case."));
	EXPECT_FALSE(_emptyStrIntCache.contains("key"));
	EXPECT_FALSE(_emptyStrIntCache.contains("k"));
	EXPECT_FALSE(_emptyStrIntCache.contains(
		"Very long string with spaces and mixed case."));
}
 ...
 * returns: nothing
 */
void processTestCase(CacheManagerPtr &cacheManager,
					 const std::string &testCaseName,
					 const json &testCaseArray) {
...
int main(int argc, char **argv) {
	...
	for (size_t i = 0; i < data["cacheManager"].size(); ++i) {
		...
		for (json::const_iterator it = testCase.begin(); it != testCase.end();
			 ++it) {
